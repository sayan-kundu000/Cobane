...
class ProjectRepository(BaseRepository[Project]):
    ...
    async def list_by_owner(self, owner_id_val: int, skip: int = 0, limit: int = 20) -> List[Project]:
        ...
        result = await self.db.execute(
            select(Project).filter(Project.owner_id == owner_id_val).offset(skip).limit(limit)
        )
        return list(result.scalars().all())

    async def search_and_filter(self, params: QueryParams, owner_id: Optional[int] = None) -> Tuple[List[Project], int]:
        ...
        if params.search.q:
            ...
            query = query.filter(Project.name.ilike(keyword) | Project.description.ilike(keyword))
        ...
        if params.filters.language:
            ...
            query = query.join(UploadedSource).filter(UploadedSource.language.ilike(params.filters.language)).distinct()
